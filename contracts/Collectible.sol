    ...
    ERC721Enumerable
{
    ...
    modifier validTokenURI(uint256 _tokenId, string memory _tokenURI) {
        bytes memory bTokenURI = bytes(_tokenURI);
        ...
        if (_tokenId != 0) {
            bytes memory bExistingTokenURI = bytes(tokenURI(_tokenId));

            if (keccak256(bTokenURI) != keccak256(bExistingTokenURI)) {
    ...
    function safeMint(address _to, string memory _tokenURI)
        public
        onlyOwner
        validTokenURI(_tokenIdCounter.current(), _tokenURI)
    {
    ...
    function setTokenURI(uint256 _tokenId, string memory _tokenURI)
        public
        onlyOwner
        validTokenURI(_tokenId, _tokenURI)
    {
        require(
